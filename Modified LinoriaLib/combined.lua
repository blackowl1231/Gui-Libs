...
EZ.ScreenGui = Instance.new('ScreenGui')
ProtectGui(EZ.ScreenGui)
EZ.ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
EZ.ScreenGui.Parent = CoreGui
EZ.ScreenGui.DisplayOrder = 999

EZ.OutlineGui = Instance.new('ScreenGui')
ProtectGui(EZ.OutlineGui)
EZ.OutlineGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
EZ.OutlineGui.Parent = CoreGui
EZ.OutlineGui.DisplayOrder = 1000

EZ.CursorGui = Instance.new('ScreenGui')
ProtectGui(EZ.CursorGui)
EZ.CursorGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
EZ.CursorGui.Parent = CoreGui
EZ.CursorGui.DisplayOrder = 1001

EZ.MenuBlur = Instance.new('BlurEffect')
EZ.MenuBlur.Name = '\0'
EZ.MenuBlur.Size = 0
EZ.MenuBlur.Enabled = false

EZ.MenuColor = Instance.new('ColorCorrectionEffect')
EZ.MenuColor.Name = '\0'
EZ.MenuColor.Enabled = false

EZ.MenuDimGui = Instance.new('ScreenGui')
ProtectGui(EZ.MenuDimGui)
EZ.MenuDimGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
EZ.MenuDimGui.IgnoreGuiInset = true
EZ.MenuDimGui.DisplayOrder = 997
EZ.MenuDimGui.Parent = CoreGui

EZ.MenuDim = Instance.new('Frame')
EZ.MenuDim.BackgroundColor3 = Color3.new(0, 0, 0)
EZ.MenuDim.BackgroundTransparency = 1
EZ.MenuDim.BorderSizePixel = 0
EZ.MenuDim.Size = UDim2.fromScale(1, 1)
EZ.MenuDim.ZIndex = 0
EZ.MenuDim.Visible = false
EZ.MenuDim.Parent = EZ.MenuDimGui

function EZ:EnsureFolders()
    ensurefolder(self.FolderName)
    ensurefolder(self.FolderName .. '/cache')
    ensurefolder(self.FolderName .. '/themes')
    ensurefolder(self.FolderName .. '/' .. self.Game)
    ensurefolder(self.FolderName .. '/' .. self.Game .. '/config')
end

function EZ:ReadCache()
    local path = self.FolderName .. '/cache/autoload.dat'
...
function EZ:WriteCache(data)
    ensurefolder(self.FolderName)
    ensurefolder(self.FolderName .. '/cache')
    writefile(self.FolderName .. '/cache/autoload.dat', HttpService:JSONEncode(data))
end
...
function EZ:SafeCallback(f, ...)
    ...
    if not EZ.NotifyOnError then return f(...) end
    local success, event = pcall(f, ...)
    if not success then
        local _, i = event:find(":%d+: ")
        if not i then return EZ:Notify(event) end
        return EZ:Notify(event:sub(i + 1), 3)
    end
end

function EZ:AttemptSave()
    if EZ.SaveManager then EZ.SaveManager:Save() end
end
...
function EZ:CreateLabel(Properties, IsHud)
    ...
    EZ:AddToRegistry(_Instance, { TextColor3 = 'FontColor' }, IsHud)
...
function EZ:MakeDraggable(Instance, Cutoff, IgnoreForced)
    ...
    Instance.InputBegan:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
            if IgnoreForced == true and EZ.CantDragForced == true then return end
...
function EZ:MakeResizable(Instance, MinSize)
    ...
    if EZ.IsMobile ~= true then
        Grip.MouseButton1Down:Connect(function()
            ...
            Grip.Size = UDim2.fromOffset(EZ.ScreenGui.AbsoluteSize.X, EZ.ScreenGui.AbsoluteSize.Y)
            Grip.Position = UDim2.new()
            GripCorner.Parent = nil
            Grip.Parent = EZ.ScreenGui
        end)

        Grip.MouseMoved:Connect(function()
            if not DragOffset then return end
        ...
        Grip.InputBegan:Connect(function(Input)
            if Input.UserInputType ~= Enum.UserInputType.Touch or DragOffset then return end
            DragOffset = Vector2.new(
        ...
        Grip.InputChanged:Connect(function(Input)
            if Input.UserInputType ~= Enum.UserInputType.Touch or not DragOffset then return end
...
function EZ:UpdateBackground(Mode)
    ...
    EZ.MenuDim.BackgroundColor3 = B.Color
    ...
    if Mode == 'snap' then
        if EZ.MenuColor.Parent then
            EZ.MenuColor.Contrast, EZ.MenuColor.Saturation, EZ.MenuColor.Brightness = Contrast, Saturation, Brightness
        end
        if EZ.MenuBlur.Parent then EZ.MenuBlur.Size = Blur end
        EZ.MenuDim.BackgroundTransparency = Dim
        return
    end

    local Info = TweenInfo.new(EZ.MenuFadeTime, Enum.EasingStyle.Linear)
    if EZ.MenuColor.Parent then
        TweenService:Create(EZ.MenuColor, Info, {
            Contrast = Contrast, Saturation = Saturation, Brightness = Brightness,
        }):Play()
    end
    if EZ.MenuBlur.Parent then
        TweenService:Create(EZ.MenuBlur, Info, { Size = Blur }):Play()
    end
    TweenService:Create(EZ.MenuDim, Info, { BackgroundTransparency = Dim }):Play()
end

function EZ:UpdateKeybindFrame()
    for _, Option in next, EZ.Options do
...
function EZ:AddToolTip(InfoStr, HoverInstance)
    ...
    EZ:AddToRegistry(Tooltip, { BackgroundColor3 = 'MainColor', BorderColor3 = 'OutlineColor' })
    EZ:AddToRegistry(Label, { TextColor3 = 'FontColor' })

    local IsHovering = false
    HoverInstance.MouseEnter:Connect(function()
        if EZ:MouseIsOverOpenedFrame() then return end
        IsHovering = true
...
function EZ:OnHighlight(HighlightInstance, Instance, Properties, PropertiesDefault, Condition)
    ...
    local function ApplyHighlight()
        if Condition and not Condition() then ApplyDefault(); return end
        local Reg = EZ.RegistryMap[Instance]
...
function EZ:FireEvent(Name, ...)
    if EZ.Events[Name] then EZ.Events[Name]:Fire(...) end
end

function EZ:OnEvent(Name)
    if not EZ.Events[Name] then EZ:CreateEvent(Name) end
...
EZ:GiveSignal(InputService.InputBegan:Connect(function(Input, ...)
    if not EZ.Visible then return end
...
function EZ:AddContextMenu(Anchor, Trigger)
    ...
    EZ:AddToRegistry(ContextMenu.Inner, { BackgroundColor3 = 'BackgroundColor', BorderColor3 = 'OutlineColor' })
...
EZ:GiveSignal(EZ.ScreenGui.DescendantRemoving:Connect(function(Instance)
    if EZ.RegistryMap[Instance] then EZ:RemoveFromRegistry(Instance) end
end))
...
do
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        function ColorPicker:Remove()
            ...
            if Index then table.remove(Pickers, Index) end
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        EZ:AddToRegistry(PickInner, { BackgroundColor3 = 'BackgroundColor', BorderColor3 = 'OutlineColor' })
        ...
        EZ:AddToRegistry(ModeSelectInner, { BackgroundColor3 = 'BackgroundColor', BorderColor3 = 'OutlineColor' })
        ...
        local update = function(State)
            ...
            if ModeText ~= 'Always' and KeyPicker.Override then ModeText = 'Override' end
        ...
        function KeyPicker:OverrideState(State)
            if KeyPicker.Override == State then return end
        ...
        function KeyPicker:GetState()
            ...
            if State and KeyPicker.Override then KeyPicker:OverrideState(false) end
        ...
        function KeyPicker:Remove()
            EZ.Options[Idx] = nil
            for _, Connection in next, KeyPicker.Connections do Connection:Disconnect() end
...
do
    ...
    function Funcs:AddContainer(Info)
        ...
        EZ:AddToRegistry(Inner, { BackgroundColor3 = 'BackgroundColor', BorderColor3 = 'OutlineColor' })
        ...
        function Element:GetOuter() return Outer end
        function Element:GetInner() return Inner end
        function Element:GetSize() return Inner.AbsoluteSize end

        local Instances = {}
        table.insert(Instances, Groupbox:AddBlank(5))

        function Element:Remove()
            for _, Instance in next, Instances do Instance:Destroy() end
            Outer:Destroy()
            table.clear(Element)
            Groupbox:Resize()
        end
    ...
    function Funcs:AddButton(...)
        ...
        local function InitEvents(Button)
            ...
            local function ValidateClick(Input)
                if EZ:MouseIsOverOpenedFrame() then return false end
    ...
    function Funcs:AddDivider()
        ...
        EZ:AddToRegistry(DividerOuter, { BorderColor3 = 'Black' })
        EZ:AddToRegistry(DividerInner, { BackgroundColor3 = 'MainColor', BorderColor3 = 'OutlineColor' })
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        EZ:AddToRegistry(TextBoxInner, { BackgroundColor3 = 'MainColor', BorderColor3 = 'OutlineColor' })
        ...
        local Container2 = EZ:Create('Frame', {
            BackgroundTransparency = 1,
            ClipsDescendants = true,
            Position = UDim2.new(0, 5, 0, 0),
            Size = UDim2.new(1, -5, 1, 0),
        ...
        function Textbox:Remove()
            for _, Instance in next, Textbox.Instances do Instance:Destroy() end
            EZ.Options[Idx] = nil
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        EZ:AddToRegistry(ToggleOuter, { BorderColor3 = 'Black' })
        ...
        EZ:AddToRegistry(ToggleInner, { BackgroundColor3 = 'MainColor', BorderColor3 = 'OutlineColor' })
        ...
        function Toggle:SetValue(Bool)
            if Toggle.SettingValue then return end
        ...
        function Toggle:Remove()
            for _, Instance in next, Toggle.Instances do Instance:Destroy() end
            EZ.Toggles[Idx] = nil
            ToggleOuter:Destroy()
            table.clear(Toggle)
            Groupbox:Resize()
        ...
        ToggleRegion.InputBegan:Connect(function(Input)
            if EZ.IsDragging then return end
    ...
    function Funcs:AddSlider(Idx, Info, SliderParent)
        ...
        EZ:AddToRegistry(SliderOuter, { BorderColor3 = 'Black' })
        ...
        EZ:AddToRegistry(SliderInner, { BackgroundColor3 = 'MainColor', BorderColor3 = 'OutlineColor' })
        ...
        EZ:AddToRegistry(Fill, { BackgroundColor3 = 'AccentColor', BorderColor3 = 'AccentColorDark' })
        ...
        EZ:AddToRegistry(HideBorderRight, { BackgroundColor3 = 'AccentColor' })
        ...
        local get_count = function()
            ...
            repeat
                ...
                if (parent) then count = count + 1 end
            until not parent
            return count
        end
        ...
        SliderInner.InputBegan:Connect(function(Input)
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and not EZ:MouseIsOverOpenedFrame() then
                ...
                while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                    ...
                    Slider.Value = nValue
                    Slider:Display()
                    if nValue ~= OldValue then FireChanged(nValue) end
                    RenderStepped:Wait()
                end
                EZ:AttemptSave()
            end
        ...
        local get_slider = function(count)
            local slider = Slider
            for i = 1, count-1 do slider = slider.Parent end
            return slider
        end

        local wanted_size = (Groupbox.Container.AbsoluteSize.X) / size
        wanted_size = wanted_size + size - 2
        ...
        function Slider:Remove()
            for _, Instance in next, Slider.Instances do Instance:Destroy() end
            EZ.Options[Idx] = nil
            SliderOuter:Destroy()
            table.clear(Slider)
            Groupbox:Resize()
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        EZ:AddToRegistry(DropdownOuter, { BorderColor3 = 'Black' })
        ...
        EZ:AddToRegistry(DropdownInner, { BackgroundColor3 = 'MainColor', BorderColor3 = 'OutlineColor' })
        ...
        EZ:AddToRegistry(ListInner, { BackgroundColor3 = 'MainColor', BorderColor3 = 'OutlineColor' })
        ...
        EZ:AddToRegistry(Scrolling, { ScrollBarImageColor3 = 'AccentColor' })
        ...
        function Dropdown:BuildDropdownList()
            ...
            for Idx, Value in next, Values do
                ...
                EZ:AddToRegistry(Button, { BackgroundColor3 = 'MainColor', BorderColor3 = 'OutlineColor' })
        ...
        function Dropdown:OpenDropdown()
            if EZ.IsMobile then EZ.CanDrag = false end
        ...
        function Dropdown:CloseDropdown()
            if EZ.IsMobile then EZ.CanDrag = true end
        ...
        function Dropdown:Remove()
            for _, Instance in next, Dropdown.Instances do Instance:Destroy() end
            EZ.Options[Idx] = nil
    ...
    function Funcs:AddDependencyBox()
        ...
        function Depbox:Remove()
            ...
            if Index then table.remove(EZ.DependencyBoxes, Index) end
            table.clear(Depbox)
            Holder:Destroy()
            Groupbox:Resize()
        end
...
do
    ...
    EZ:AddToRegistry(WatermarkInner, { BorderColor3 = 'AccentColor' })
    ...
    EZ:AddToRegistry(KeybindInner, { BackgroundColor3 = 'MainColor', BorderColor3 = 'OutlineColor' }, true)
    ...
    EZ:AddToRegistry(ColorFrame, { BackgroundColor3 = 'AccentColor' }, true)
...
function EZ:CreatePopout(Config)
    ...
    EZ:AddToRegistry(BodyOuter, { BackgroundColor3 = 'BackgroundColor', BorderColor3 = 'OutlineColor' })
    ...
    EZ:AddToRegistry(ContainerOuter, { BackgroundColor3 = 'BackgroundColor', BorderColor3 = 'OutlineColor' })
...
function EZ:CreateWindow(...)
    ...
    EZ:AddToRegistry(MainSectionOuter, { BackgroundColor3 = 'BackgroundColor', BorderColor3 = 'OutlineColor' })
    ...
    EZ:AddToRegistry(MainSectionInner, { BackgroundColor3 = 'BackgroundColor' })
    ...
    EZ:AddToRegistry(TabArea, { BorderColor3 = 'OutlineColor' })
    ...
    function Window:AddTab(Name)
        ...
        EZ:AddToRegistry(TabButton, { BackgroundColor3 = 'BackgroundColor', BorderColor3 = 'OutlineColor' })
        ...
        EZ:AddToRegistry(TabButtonBorder, { BorderColor3 = 'OutlineColor' })
        ...
        EZ:AddToRegistry(TabAccent, { BackgroundColor3 = 'AccentColor' })
        ...
        EZ:AddToRegistry(TabFrame, { BorderColor3 = 'OutlineColor' })
        ...
        EZ:AddToRegistry(WarningBox, { BackgroundColor3 = 'MainColor', BorderColor3 = 'OutlineColor' })
        ...
        if EZ.IsMobile then
            ...
            local function GuardScroll(Key)
                return function()
                    ...
                    task.wait(0.15)
                    if LastScroll[Key] == Stamp then EZ.CanDrag = true end
                end
            end
        ...
        function Tab:AddGroupbox(Info)
            ...
            EZ:AddToRegistry(BoxOuter, { BackgroundColor3 = 'BackgroundColor', BorderColor3 = 'OutlineColor' })
            ...
            EZ:AddToRegistry(BoxInner, { BackgroundColor3 = 'BackgroundColor' })
        ...
        function Tab:AddTabbox(Info)
            ...
            EZ:AddToRegistry(TabboxButtons, { BorderColor3 = 'OutlineColor' })
            ...
            function Tabbox:AddTab(Name)
                ...
                EZ:AddToRegistry(Button, { BackgroundColor3 = 'MainColor' })
                ...
                EZ:AddToRegistry(Block, { BackgroundColor3 = 'BackgroundColor' })
                ...
                EZ:AddToRegistry(Container, { BorderColor3 = 'OutlineColor' })
    ...
    function EZ:Toggle()
        ...
        for _, Element in next, EZ.Options do
            task.spawn(function()
        ...
        if Toggled then
            ...
            task.spawn(function()
                ...
                local Cursor = Instance.new("ImageLabel", EZ.CursorGui)
                Cursor.Image = "http://www.roblox.com/asset/?id=4292970642"
                Cursor.BackgroundTransparency = 1
                Cursor.ZIndex = 100

                local CursorOutline = Instance.new("ImageLabel", EZ.CursorGui)
                ...
                local Shown = nil
                while Toggled and EZ.ScreenGui.Parent do
    ...
    function EZ:SetVisible(Bool)
        if Bool ~= Toggled then EZ:Toggle() end
    end
...
do
    ...
    ThemeManager.BuiltInThemes = {
        ['Default']      = MakeTheme('ffffff', '1a1817', '1300ff', '161413', '2a2624', 'e50000'),
    ...
    local function GetFlag(Key)
        return EZ.Options[Key] or EZ.Toggles[Key]
    end
    ...
    function ThemeManager:ApplyTheme(Name)
        ...
        if not Data and BuiltIn then Data = BuiltIn[2] end
    ...
    function ThemeManager:ThemeUpdate()
        ...
        for _, Key in next, self.ColorKeys do
            if EZ.Options[Key] then
                Library[Key] = EZ.Options[Key].Value
            end
        end
    ...
    function ThemeManager:LoadDefault()
        ...
        if IsBuiltIn then
            EZ.Options.ThemeManager_ThemeList:SetValue(Name)
        else
    ...
    function ThemeManager:GetCustomTheme(File)
        ...
        if not File:find('%.json$') then File = File .. '.json' end
        local Path = EZ.FolderName .. '/themes/' .. File
    ...
    function ThemeManager:SaveCustomTheme(File)
        ...
        writefile(EZ.FolderName .. '/themes/' .. File, HttpService:JSONEncode(Data))
    end

    function ThemeManager:Delete(File)
        if not File then return false, 'no theme file is selected' end
        if not File:find('%.json$') then File = File .. '.json' end
        local Path = EZ.FolderName .. '/themes/' .. File
    ...
    function ThemeManager:ReloadCustomThemes()
        local Files = listfiles(EZ.FolderName .. '/themes')
        local List = {}
    ...
    function ThemeManager:BuildMenuTab(Tab)
        ...
        Library.ToggleKeybind = EZ.Options.MenuKeybind
        ...
        KeybindMenuBox:SetupDependencies({ { EZ.Toggles.KeybindMenu, true } })

        EZ.Toggles.KeybindMenu:OnChanged(function(Value)
            Library.KeypickerListVisible = Value
            Library:UpdateKeybindFrame()
        end)

        EZ.Options.KeybindMenuTransparency:OnChanged(function(Value)
            Library.KeybindMenuTransparency = Value / 100
            Library:UpdateKeybindMenu()
        end)

        EZ.Options.KeybindMenuMode:OnChanged(function(Value)
    ...
    function ThemeManager:BuildNotificationsTab(Tab)
        ...
        ClipBox:SetupDependencies({ { EZ.Toggles.NotificationClips, true } })
        ...
        ForceColorsBox:AddButton({
            ...
            Func = function()
                EZ.Options.NotificationForcedAccent:SetValueRGB(Library.AccentColor)
                EZ.Options.NotificationForcedOutline:SetValueRGB(Library.OutlineColor)
                EZ.Options.NotificationForcedText:SetValueRGB(Library.FontColor)
            end,
        })
        ForceColorsBox:SetupDependencies({ { EZ.Toggles.NotificationForceColors, true } })
        ...
        EZ.Toggles.NotificationClips:OnChanged(function(Value)
            S.Clips = Value
            Library:UpdateNotifications()
        end)

        EZ.Options.NotificationClipsDistance:OnChanged(function(Value)
            S.ClipsDistance = Value
            Library:UpdateNotifications()
        end)

        local function UpdateForcedColors()
            if EZ.Toggles.NotificationForceColors.Value then
                S.OverrideColor = function()
                    return Library.MainColor,
                        EZ.Options.NotificationForcedAccent.Value,
                        EZ.Options.NotificationForcedOutline.Value,
                        EZ.Options.NotificationForcedText.Value
                end
            else
                S.OverrideColor = nil
            end
        ...
        EZ.Toggles.NotificationForceColors:OnChanged(UpdateForcedColors)
        EZ.Options.NotificationForcedAccent:OnChanged(UpdateForcedColors)
        EZ.Options.NotificationForcedOutline:OnChanged(UpdateForcedColors)
        EZ.Options.NotificationForcedText:OnChanged(UpdateForcedColors)
        UpdateForcedColors()

        EZ.Options.NotificationPositionX:OnChanged(function(Value)
            S.PositionX = Value / 100
            Library:UpdateNotifications()
        end)

        EZ.Options.NotificationPositionY:OnChanged(function(Value)
            S.PositionY = Value / 100
            Library:UpdateNotifications()
        end)

        EZ.Options.NotificationTransparency:OnChanged(function(Value)
            S.Transparency = 1 - Value / 100
        end)

        EZ.Options.NotificationAnchorStyle:OnChanged(function(Value)
            S.Anchor = Value
            Library:UpdateNotifications()
        end)

        EZ.Options.NotificationBarStyle:OnChanged(function(Value)
            S.BarSide = BarSides[Value] or 'Bottom'
        end)

        EZ.Options.NotificationStyleSortOrder:OnChanged(function(Value)
            S.SortOrder = Value
        end)
    end
    ...
    function ThemeManager:BuildBackgroundTab(Tab)
        ...
        for Flag, Field in next, self.BackgroundKeys do
            if EZ.Options[Flag] then
                EZ.Options[Flag]:OnChanged(function(Value)
                    B[Field] = Value
    ...
    function ThemeManager:CreateThemeManager(Container)
        ...
        Groupbox:AddButton('Set default', function()
            self:SaveDefault(EZ.Options.ThemeManager_ThemeList.Value)
            self.Library:Notify(string.format('Set default theme to %q', EZ.Options.ThemeManager_ThemeList.Value))
        end):AddButton('Reset default', function()
            self:ResetDefault()
            self.Library:Notify('Reset default theme')
        end)

        EZ.Options.ThemeManager_ThemeList:OnChanged(function()
            self:ApplyTheme(EZ.Options.ThemeManager_ThemeList.Value)
        end)
        ...
        Groupbox:AddButton('Create theme', function()
            local Name = EZ.Options.ThemeManager_CustomThemeName.Value
            if not Name or Name:gsub(' ', '') == '' then
                return self.Library:Notify('Invalid file name for theme (empty)', 3)
            end

            local Display = Name:gsub('%.json$', '')
            if isfile(EZ.FolderName .. '/themes/' .. Display .. '.json') then
                return self.Library:Notify(string.format('Theme %q already exists, use the overwrite button to replace it', Display), 3)
            end

            self:SaveCustomTheme(Name)
            EZ.Options.ThemeManager_CustomThemeList:SetValues(self:ReloadCustomThemes())
            EZ.Options.ThemeManager_CustomThemeList:SetValue(Display)
            self.Library:Notify(string.format('Created theme %q', Display))
        end):AddButton('Load theme', function()
            self:ApplyTheme(EZ.Options.ThemeManager_CustomThemeList.Value)
        end)

        Groupbox:AddButton({ Text = 'Overwrite theme', DoubleClick = true, Func = function()
            self:SaveCustomTheme(EZ.Options.ThemeManager_CustomThemeList.Value)
        end }):AddButton({ Text = 'Delete theme', DoubleClick = true, Func = function()
            local Name = EZ.Options.ThemeManager_CustomThemeList.Value
            ...
            EZ.Options.ThemeManager_CustomThemeList:SetValues(self:ReloadCustomThemes())
            EZ.Options.ThemeManager_CustomThemeList:SetValue(nil)
        end })
        ...
        for _, Key in next, self.ColorKeys do
            if EZ.Options[Key] then
                EZ.Options[Key]:OnChanged(function()
                    self:ThemeUpdate()
                end)
            end
        end
...
do
    ...
    SaveManager.Parser = {
        Toggle = {
            ...
            Load = function(Idx, Data)
                if EZ.Toggles[Idx] then
                    EZ.Toggles[Idx]:SetValue(Data.value)
                end
            end,
        },
        Slider = {
            ...
            Load = function(Idx, Data)
                if EZ.Options[Idx] then
                    EZ.Options[Idx]:SetValue(Data.value)
                end
            end,
        },
        Dropdown = {
        ...
        ColorPicker = {
            ...
            Load = function(Idx, Data)
                if EZ.Options[Idx] then
                    EZ.Options[Idx]:SetValueRGB(Color3.fromHex(Data.value), Data.transparency)
                end
            end,
        },
        KeyPicker = {
            ...
            Load = function(Idx, Data)
                if EZ.Options[Idx] then
                    EZ.Options[Idx]:SetValue({ Data.key, Data.mode })
                end
            end,
        },
        Input = {
            ...
            Load = function(Idx, Data)
                if EZ.Options[Idx] and type(Data.text) == 'string' then
                    EZ.Options[Idx]:SetValue(Data.text)
                end
            end,
        },
    }
    ...
    function SaveManager:ConfigFolder()
        return EZ.FolderName .. '/' .. EZ.Game .. '/config'
    end
    ...
    function SaveManager:Save(Name, NoWrite)
        ...
        for Idx, Object in next, EZ.Toggles do
            if self.Ignore[Idx] then continue end
            table.insert(Data.objects, self.Parser[Object.Type].Save(Idx, Object))
        end

        for Idx, Object in next, EZ.Options do
            if not self.Parser[Object.Type] then continue end
            if self.Ignore[Idx] then continue end
            table.insert(Data.objects, self.Parser[Object.Type].Save(Idx, Object))
        end
    ...
    function SaveManager:BuildConfigSection(Tab)
        ...
        Section:AddButton('Create config', function()
            local Name = EZ.Options.SaveManager_ConfigName.Value
            if Name:gsub(' ', '') == '' then
            ...
            EZ.Options.SaveManager_ConfigList:SetValues(self:RefreshConfigList())
            EZ.Options.SaveManager_ConfigList:SetValue(nil)
            EZ.Options.SaveManager_ConfigCopyList:SetValues(self:RefreshConfigList())
        end):AddButton('Load config', function()
            local Name = EZ.Options.SaveManager_ConfigList.Value
            local Success, Err = self:Load(Name)
            if not Success then
        ...
        Section:AddButton({ Text = 'Overwrite config', DoubleClick = true, Func = function()
            ...
            EZ.Options.SaveManager_ConfigList:SetValue(nil)
            EZ.Options.SaveManager_ConfigCopyList:SetValues(self:RefreshConfigList())
            EZ.Options.SaveManager_ConfigCopyList:SetValue(nil)
        end })

        Section:AddButton('Refresh list', function()
            local List = self:RefreshConfigList()
            EZ.Options.SaveManager_ConfigList:SetValues(List)
            EZ.Options.SaveManager_ConfigList:SetValue(nil)
            EZ.Options.SaveManager_ConfigCopyList:SetValues(List)
            EZ.Options.SaveManager_ConfigCopyList:SetValue(nil)
        end)
        ...
        Section:AddInput('SaveManager_ConfigImport', {
            ...
            Callback = function(Raw)
                ...
                if not Success or type(Decoded) ~= 'table' or type(Decoded.objects) ~= 'table' then
                    EZ.Options.SaveManager_ConfigImport:SetValue('')
        ...
        Section:AddButton('Copy config to clipboard', function()
            ...
            local Name = EZ.Options.SaveManager_ConfigCopyList.Value
            if not Name then
...
task.spawn(function()
    EZ.SaveManager:LoadAutoloadConfig()
end)
